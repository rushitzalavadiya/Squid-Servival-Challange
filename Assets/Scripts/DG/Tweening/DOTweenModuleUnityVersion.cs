...
namespace DG.Tweening
{
    public static class DOTweenModuleUnityVersion
    {
            ...
            int propertyID, float duration)
        {
            ...
            var tweenerCore = DOTween.To(() => target.GetTextureOffset(propertyID),
                delegate(Vector2 x) { target.SetTextureOffset(propertyID, x); }, endValue, duration);
            ...
            int propertyID, float duration)
        {
            ...
            var tweenerCore = DOTween.To(() => target.GetTextureScale(propertyID),
                delegate(Vector2 x) { target.SetTextureScale(propertyID, x); }, endValue, duration);
            tweenerCore.SetTarget(target);
            return tweenerCore;
        }
    }
