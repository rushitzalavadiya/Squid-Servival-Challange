...
namespace DG.Tweening
{
    ...
    public class DOTweenAnimation : ABSAnimationComponent
    {
        ...
        public void CreateTween()
        {
            ...
            if (target == null || tweenGO == null)
            {
                if (targetIsSelf && target == null)
                    Debug.LogWarning(
                        $"{this.gameObject.name} :: This DOTweenAnimation's target is NULL, because the animation was created with a DOTween Pro version older than 0.9.255. To fix this, exit Play mode then simply select this object, and it will update automatically",
                        this.gameObject);
                else
                    Debug.LogWarning(
                        $"{this.gameObject.name} :: This DOTweenAnimation's target/GameObject is unset: the tween will not be created.",
                        this.gameObject);
                return;
            }

            if (forcedTargetType != 0) targetType = forcedTargetType;
            if (this.targetType == TargetType.Unset) targetType = TypeToDOTargetType(target.GetType());
            switch (animationType)
            {
                case AnimationType.Move:
                    if (useTargetAsV3)
                    {
                        ...
                        {
                            Debug.LogWarning(
                                $"{this.gameObject.name} :: This tween's TO target is NULL, a Vector3 of (0,0,0) will be used instead",
                                this.gameObject);
                            endValueV3 = Vector3.zero;
                        }
                        else if (targetType == TargetType.RectTransform)
                        {
                            var rectTransform = endValueTransform as RectTransform;
                            if (rectTransform == null)
                            {
                                Debug.LogWarning(
                                    $"{this.gameObject.name} :: This tween's TO target should be a RectTransform, a Vector3 of (0,0,0) will be used instead",
                                    this.gameObject);
                                endValueV3 = Vector3.zero;
                            }
                            else
                            {
                                var rectTransform2 = target as RectTransform;
                                if (rectTransform2 == null)
                                    Debug.LogWarning(
                                        $"{this.gameObject.name} :: This tween's target and TO target are not of the same type. Please reassign the values",
                                        this.gameObject);
                                else
                                    endValueV3 =
                                        DOTweenModuleUI.Utils.SwitchToRectTransform(rectTransform, rectTransform2);
                            }
                        }
                        else
                        {
            ...
            var gameObject = targetIsSelf || !tweenTargetIsTargetGO ? this.gameObject : targetGO;
            tween.SetTarget(gameObject).SetDelay(delay).SetLoops(loops, loopType)
                .SetAutoKill(autoKill)
                .OnKill(delegate { tween = null; });
        ...
        {
            _playCount = -1;
            var components = gameObject.GetComponents<DOTweenAnimation>();
            for (var num = components.Length - 1; num > -1; num--)
            {
                var tween = components[num].tween;
                if (tween != null && tween.IsInitialized()) components[num].tween.Rewind();
            }
        }
        ...
        {
            var components = GetComponents<DOTweenAnimation>();
            ...
            do
            {
                ...
                return;
            } while (!(dOTweenAnimation != null) || dOTweenAnimation.tween == null ||
                     dOTweenAnimation.tween.IsPlaying() || dOTweenAnimation.tween.IsComplete());
        ...
        public List<Tween> GetTweens()
        {
            ...
            var components = GetComponents<DOTweenAnimation>();
            foreach (var dOTweenAnimation in components) list.Add(dOTweenAnimation.tween);
            return list;
        }
        ...
        public static TargetType TypeToDOTargetType(Type t)
        {
            ...
            if (num != -1) text = text.Substring(num + 1);
            if (text.IndexOf("Renderer") != -1 && text != "SpriteRenderer") text = "Renderer";
            if (text == "RawImage") text = "Image";
        ...
        {
            if (!targetIsSelf) return targetGO;
            return gameObject;
        }
        ...
        private void ReEvaluateRelativeTween()
        {
            ...
            if (tweenGO == null)
                Debug.LogWarning(
                    $"{gameObject.name} :: This DOTweenAnimation's target/GameObject is unset: the tween will not be created.",
                    gameObject);
